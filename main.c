...
static uint8_t  binaryValues[] = {0b10101010, 0b01010101, 0b11001100,
...
int main(void)
{
  ...
  /**write_to_address(0,10101010);
  write_to_address(1,01010101);
  write_to_address(2,11001100);
  write_to_address(3,00110011);
  write_to_address(4,11110000);
  write_to_address(5,00001111);**/

  numEntries = sizeof(binaryValues) / sizeof(binaryValues[0]);

  uint16_t x ;

  for (x = 0; numEntries>x;++x)
  {
	  write_to_address(x,binaryValues[x]);
  }
  ...
  while (1)
  {
	...
	// TODO: Check button PA0; if pressed, change timer delay
	  if (!(LL_GPIO_ReadInputPort(Button0_GPIO_Port) & Button0_Pin))
		{
		  htim16.Init.Period = (1000-1)* !sub+(500-1)*sub;
		  sub = !sub;
...
void TIM16_IRQHandler(void)
{
	...
	if (i <= 5)
	{
		myData = read_from_address(i);

		if (myData != binaryValues[i])
		 {
